...
import com.swapna.foodapp.presentation.common.fakes.fakeRestaurant
import com.swapna.foodapp.presentation.common.fakes.getHomeDataFakeCategories
import com.swapna.foodapp.presentation.common.fakes.getHomeDataFakeCollections
import com.swapna.foodapp.presentation.common.fakes.getHomeDataFakeRestaurants
...
class GetHomeDataUseCaseSpec : DescribeSpec({
        ...
        context("all repositories return data successfully") {
            ...
            beforeEach {
                ...
                every { restaurantRepository.getNearbyRestaurants() } returns
                        flowOf(Result.success(getHomeDataFakeRestaurants))
            }
            ...
            it("should return HomeData with all three lists populated") {
                ...
                result.getOrNull()!!.collections.size shouldBe HOME_COLLECTION_COUNT_1
                result.getOrNull()!!.categories.size shouldBe HOME_CATEGORY_COUNT_2
            ...
            it("collection title should match first collection") {
                ...
                result.getOrNull()!!.collections.first().title shouldBe COLLECTION_TITLE_FIRST
            }
            ...
            it("first restaurant id should match") {
                ...
                result.getOrNull()!!.restaurants.first().id shouldBe HOME_RESTAURANT_ID_1
            }
        }
                ...
                Result.success(
                    listOf(
                        fakeRestaurant(HOME_RESTAURANT_ID_1).copy(
                            locality = LOC_GREATER_KORA,
                            address = ADDR_GREATER_KORA,
                        )
                    )
                )
            )
        ...
        it("lowercase koramangala matches KORAMANGALA locality") {
            ...
            every { restaurantRepository.getNearbyRestaurants() } returns flowOf(
                Result.success(
                    listOf(
                        fakeRestaurant(HOME_RESTAURANT_ID_1).copy(
                            locality = LOC_KORAMANGALA_UPPER,
                            address = ADDR_UPPER_KORA,
                        )
                    )
                )
            )
            ...
            val result = GetHomeDataUseCase(restaurantRepository)
                .invoke(LOC_KORAMANGALA_LOWER).first().getOrNull()!!
    ...
    context("selectedLocation has no matching restaurants — Jakkur") {
        ...
        it("availableAreas contains all distinct localities sorted alphabetically") {
            ...
            result.availableAreas shouldBe
                    listOf(LOC_HSR, LOC_INDIRANAGAR, LOC_KORAMANGALA)
        }
        ...
        it("availableAreas size equals distinct locality count") {
            ...
            result.availableAreas.size shouldBe AVAILABLE_AREAS_COUNT_3
        }
    }
    ...
    context("NOT_SERVICEABLE for out-of-city location — Mumbai") {
        it("returns NOT_SERVICEABLE with all local areas as suggestions") {
            ...
            val result = GetHomeDataUseCase(restaurantRepository)
                .invoke(LOC_MUMBAI).first().getOrNull()!!
        ...
        it("availableAreas has no duplicates and is sorted") {
            ...
            every { restaurantRepository.getNearbyRestaurants() } returns flowOf(
                Result.success(
                    listOf(
                        koramRestaurant(HOME_RESTAURANT_ID_1),
                        koramRestaurant(HOME_RESTAURANT_ID_2),
            ...
            result.availableAreas.size shouldBe AVAILABLE_AREAS_COUNT_2
            result.availableAreas shouldBe listOf(LOC_INDIRANAGAR, LOC_KORAMANGALA)
        }
    }
})
...
fun koramRestaurant(id: String) = fakeRestaurant(id).copy(
    ...
    address = ADDR_KORAMANGALA,
)

fun indiranagarRestaurant(id: String) = fakeRestaurant(id).copy(
    locality = LOC_INDIRANAGAR,
    address = ADDR_INDIRANAGAR,
)
